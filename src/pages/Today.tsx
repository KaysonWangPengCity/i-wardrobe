...
export default function TodayPage() {
  ...
  async function feedbackOutfit(liked: boolean, worn: boolean) {
    ...
    for (const it of validItems) {
      await db.items.update(it.id, {
        affinityScore: Math.max(-10, Math.min(10, it.affinityScore + (liked ? 2 : liked === false && !worn ? 0 : 0))),
    ...
    if (worn) {
      await db.wearLog.add({
        id: uuid(),
        date: todayStr(),
        itemIds: validItems.map((it) => it.id),
        eventType: 'worn',
        createdAt: now,
      })
    }

    alert(worn ? '✓ 已记录今天的穿搭' : liked ? '👍 已收藏' : '已记录')
  }
  ...
  function swapOutfit() {
    ...
    const ok = confirm(`今天已为你推荐了 ${ranked.length} 套最佳穿搭。要基于当前衣橱重新生成吗?`)
    if (ok) {
      setRefreshing(true)
  ...
  if (loading) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center gap-4 text-muted-foreground">
  ...
  if (error) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-md flex-col items-center justify-center gap-4 p-4 text-center">
  ...
  return (
    <div className="mx-auto max-w-md p-4 pb-24">
      ...
      {mode === 'ready' && (
        <div className="mt-2">
          <Button
            ...
            type="button"
            onClick={() => {
              if (confirm('确定要重新生成今日的穿搭推荐吗?会调用 AI 并覆盖现有缓存。')) {
                setRefreshing(true)
      ...
      {/* ========== 今日 Idle:CTA 卡 (R1/R3) — 方案 A:160px 圆形大按钮(和 Capture 拍照页同视觉基因) ========== */}
      {mode === 'idle' && (
        <div className="mb-6 rounded-2xl border border-indigo-200/80 bg-indigo-100/50 p-8 text-center shadow-sm">
          <div className="mb-1 text-3xl">☀️</div>
          <h2 className="mb-1 text-lg font-semibold">今天的穿搭准备好了吗？</h2>
          <p className="mb-6 text-sm text-muted-foreground leading-relaxed">
            基于你的衣橱 + 天气 + 风格偏好生成 3 套候选
            <br />
            <span className="text-[12px]">全天本地缓存,只调用 1 次 AI</span>
          </p>
          <button
            type="button"
            onClick={!disabledReason && !generating && !loading ? handleGenerateClick : undefined}
            disabled={!!disabledReason || generating || loading}
            className={`mx-auto mb-3 flex h-40 w-40 cursor-pointer flex-col items-center justify-center rounded-full border-2 border-dashed transition-all ${
              disabledReason || generating || loading
                ? 'border-gray-300 bg-gray-100 text-gray-400 cursor-not-allowed opacity-80'
                : 'border-primary bg-primary/10 text-primary hover:bg-primary/15 hover:scale-[1.02]'
            }`}
            aria-label="生成今日推荐"
          >
            {generating ? (
            ...
            <span className="mt-2 text-sm font-medium">生成今日推荐</span>
            <span className="mt-1 text-[11px] opacity-70">点击开始 · 仅首次需 AI</span>
          </button>
          <div>
            <Button
              type="button"
          ...
          {disabledReason && (
            <p className="mt-5 rounded-md bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-700 text-left">
              ⚠️ {disabledReason}
            </p>
          )}
        </div>
    ...
    <div>
      ...
      {!readonly && (
        <div className="grid grid-cols-3 gap-2">
          <Button variant="outline" onClick={onLike}>👍 喜欢</Button>
          <Button variant="outline" onClick={onSwap} disabled={swapRefreshing}>
            {swapRefreshing ? '...' : '🔄 换一套'}
          </Button>
          <Button onClick={onWorn}>✓ 穿了</Button>
        </div>
      )}
    </div>
  )
